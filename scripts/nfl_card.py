...
for _,g in gw.iterrows():
    ...
    rows.append(dict(away=a,home=h,gid=f"{a}@{h}",date=str(g.gameday),day=str(g.weekday),
        ...
        h_off_rk=int(r.loc[h,"off_rank"]), a_off_rk=int(r.loc[a,"off_rank"]),
        h_def_rk=int(r.loc[h,"def_rank"]), a_def_rk=int(r.loc[a,"def_rank"]),
...
for cf in ["AFC","NFC"]:
    ...
    H.append(f'<details open><summary>{cf} — ranked 1 to {len(blk)}</summary>'
             + (f'<div class="none" style="margin:6px 0 0">Ranked by the model. NFL.com column: {NFLPR_NOTE}.</div>' if NFLPR else '')
             + '<div class="scroll"><table><thead><tr><th>#</th><th>Team</th><th>Div</th><th>Rec</th><th>NFL.com</th>'
    ...
    for _,t in blk.iterrows():
        ...
        H.append(f'<tr><td class="num">{int(t.crank)}</td>'
                 ...
                 f'<td class="num">{("#" + str(NFLPR[t.team])) if t.team in NFLPR else "—"}</td>'
...
for cf in ["AFC","NFC"]:
    for dv in [f"{cf} East", f"{cf} North", f"{cf} South", f"{cf} West"]:
        ...
        H.append(f'<details><summary>{dv}</summary><div class="scroll"><table><thead><tr>'
                 '<th>#</th><th>Team</th><th>Rec</th><th>NFL.com</th><th>Model</th><th>Off</th>'
...
function buildNav(v){{
 ...
 [].slice.call(document.querySelectorAll('.secnav')).forEach(function(el){{
   ...
   var all=[['s-summary','Summary'],['s-changes','What changed'],['s-notes','Game notes'],['s-experts','Expert records'],['s-cheat','Conference ranks'],
