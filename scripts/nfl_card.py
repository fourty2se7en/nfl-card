...
# The earliest week that still has an unplayed game. Computed ONCE here and used
# both to CHOOSE the week below and to CHECK that choice, so the rule and its
# check can never drift apart the way two copies of one rule eventually do
# (4.3h, 4.3n).
#
# This replaced counting forward from the last PLAYED game, int(pl.week.max())+1
# over every game with a result. Every NFL week has a Thursday game, so the
# moment Thursday night went final the card jumped to the FOLLOWING week and
# abandoned that week's Sunday and Monday slate - the days the card is actually
# read. Measured walk-forward on real closing schedules for 2023-2025: the old
# expression was correct on 162 of 369 in-season days, wrong on every Sunday and
# almost every Friday, Saturday and Monday; this one is correct on 369 of 369.
# It also returned a nonexistent week 19 once week 18 was final, which killed
# the card on the "No lines posted" exit every January.
_un = sch[sch.result.isna()]
NEXT_OPEN = int(_un.week.min()) if len(_un) else int(sch.week.max())
WEEK = NEXT_OPEN if A.week is None else A.week

# Nothing on the page used to say whether the week shown was the RIGHT week. The
# self-validation in 5.9 only asked whether the rendered week was internally
# consistent, so a wrong week rendered perfectly and shipped green for four days.
# This is the check that would have caught it on the page instead of in a run log.
if WEEK != NEXT_OPEN:
    _pending = int(((sch.week == NEXT_OPEN) & sch.result.isna()).sum())
    if A.week is None:
        issue("WARN",
              f"Card is built for Week {WEEK} but Week {NEXT_OPEN} still has "
              f"{_pending} unplayed game(s)",
              "The week shown is not the earliest week with games still to play, so "
              "this page is not the next slate up. Research committed for the open "
              "week renders nothing, and picks get written for games further out "
              "than the next kickoff.",
              "No week was given on the command line, so the week rule and this "
              "check disagree - that is a bug in nfl_card.py. Report it rather than "
              "passing --week to paper over it.", tab="card")
    else:
        issue("INFO",
              f"Week {WEEK} was asked for by hand; Week {NEXT_OPEN} is the next one "
              f"with unplayed games",
              f"This run was given an explicit week, so the card shows Week {WEEK} "
              f"while Week {NEXT_OPEN} still has {_pending} game(s) to play.",
              "Leave the workflow's week input blank to follow the schedule again.",
              tab="card")
REC = {}
...
# every final game of the season, not just this week, so the same grading
# function can settle a week whose file was written before its games finished
_fin = {f"{g.away_team}@{g.home_team}": g for _,g in
        sch[sch.result.notna()].iterrows()}
def _autograde(row):
    g = _fin.get(row.game)
...
# ---- settle any earlier week that never got graded ---------------------------
# The weekly grade only ever looked at the CURRENT week, so a week whose file was
# written before its games finished kept empty results forever: Week 1 sat at
# 0-0 all season while the page reported a season-to-date record. Same grading
# function as the live week, one algorithm, applied to every week on file.
def backfill_ledgers():
    fixed = 0
    for _f in sorted(_glob.glob(P("ledger_week*.csv"))):
        try:
            d = pd.read_csv(_f, dtype={"result": str}, keep_default_na=False)
        except Exception:
            continue
        if not {"game", "market", "pick", "result"}.issubset(d.columns):
            continue
        blank = ~d.result.str.upper().str.strip().isin(["W", "L", "P"])
        if not blank.any():
            continue
        d.loc[blank, "result"] = d[blank].apply(_autograde, axis=1)
        got = int(d.result.str.upper().str.strip().isin(["W", "L", "P"]).sum())
        if got:
            d.to_csv(_f, index=False)
            fixed += int(blank.sum() - (~d.result.str.upper().str.strip().isin(["W", "L", "P"])).sum())
    if fixed:
        print(f"  ledger backfill: {fixed} earlier pick(s) settled from final scores")
    return fixed

# ---- Ledger and results ------------------------------------------------------
# Every settled pick of the season, not just this week, and every breakdown
# computed in the browser from one row set so a filter changes every table at
# once. The rows come from the committed ledger_week*.csv files, which already
# hold the frozen pre-kickoff number and the settled result, so nothing is
# re-derived here and there is no second copy of a result to drift (4.3h).
backfill_ledgers()
_all = []
for _f in sorted(_glob.glob(P("ledger_week*.csv"))):
    try:
        _d = pd.read_csv(_f, dtype={"result": str}, keep_default_na=False)
        if {"market", "tier", "result", "game"}.issubset(_d.columns):
            _all.append(_d)
    except Exception as _e:
        issue("WARN", f"A ledger file could not be read: {os.path.basename(_f)}",
              f"Season-to-date records may be incomplete ({_e}).",
              "Check the file in the repository.", tab="ledger")
LED = pd.concat(_all, ignore_index=True) if _all else pd.DataFrame(
    columns=["week", "date", "day", "game", "market", "pick", "our_num", "market_num",
             "gap", "tier", "result", "opened_num", "opened_tier"])

MKT_FIRST = {}
try:
    _h = pd.read_csv(P("ledger_history.csv"))
    _h = _h.sort_values("taken").groupby(["week", "game", "market"]).head(1)
    MKT_FIRST = {(int(t.week), str(t.game), str(t.market)): float(t.market_num)
                 for t in _h.itertuples() if pd.notna(t.market_num)}
except Exception:
    MKT_FIRST = {}

def _row_json(y):
    """One pick, with the slices worth filtering on worked out once, here."""
    g = str(y.game)
    away, home = (g.split("@") + ["", ""])[:2]
    mk = str(y.market)
    pk = str(y.pick)
    side, venue = "", ""
    if mk == "Total":
        side = "Over" if pk.lower().startswith("over") else "Under"
    else:
        _t = canon_team(pk.split()[0]) if pk.split() else None
        side = _t or ""
        venue = "Home" if side == home else ("Road" if side == away else "")
    fav = ""
    try:
        if mk == "Spread":
            _n = float(str(pk).split()[-1])
            fav = "Favorite" if _n < 0 else ("Underdog" if _n > 0 else "Pick em")
        elif mk == "Moneyline":
            fav = "Favorite" if float(y.market_num) >= 50 else "Underdog"
    except Exception:
        fav = ""
    # Closing line value: did the MARKET move toward our number between the
    # first time we priced the game and the frozen pre-kickoff view? opened_num
    # is our own first number, not the market's, so it cannot answer this; the
    # market numbers come from the snapshot history.
    clv = None
    try:
        _first = MKT_FIRST.get((int(y.week), g, mk))
        if _first is not None:
            _our, _close = float(y.our_num), float(y.market_num)
            clv = round(abs(_our - float(_first)) - abs(_our - _close), 2)
    except Exception:
        clv = None
    res = str(y.result).strip().upper()
    return dict(wk=int(y.week), date=str(y.date), day=str(y.day), game=g,
                away=away, home=home, market=mk, pick=pk,
                our=str(y.our_num), mkt=str(y.market_num), gap=str(y.gap),
                grade=str(y.tier), res=res if res in ("W", "L", "P") else "Open",
                side=side, venue=venue, fav=fav, clv=clv)

ROWS = [_row_json(y) for y in LED.itertuples()] if len(LED) else []
_settled = [x for x in ROWS if x["res"] in ("W", "L", "P")]
print(f"  results tab: {len(ROWS)} picks on file, {len(_settled)} settled, "
      f"{len({x['wk'] for x in ROWS})} week(s)")

H.append('<div id="v-ledger" class="hide"><h2 id="s-ledger">Results, season to date</h2>')
H.append('<div class="secnav" data-own="s-ledger"></div>')
H.append('<div class="card">Every pick the card has issued this season, settled from final '
         'scores. Everything below reacts to the filters: change one and every table, the grid '
         'and the export change with it. A record is only evidence when its interval clears '
         f'{PK.BREAK_EVEN:.1f}%, the break-even of standard pricing, so each one carries its '
         '95% interval. Totals appear under both teams, so team rows add up to more than the '
         'slate.</div>')
_wks = sorted({x["wk"] for x in ROWS})
_tms = sorted({t for x in ROWS for t in (x["away"], x["home"])})
_dys = list(dict.fromkeys([x["day"] for x in ROWS]))
H.append('<details class="fpanel" open><summary>Filters</summary><div class="filters">')
def _fgroup(label, cls, vals, short=False):
    H.append(f'<div class="fg"><b>{label}</b>')
    for v in vals:
        lab = str(v)[:3] if short else v
        H.append(f'<label><input type="checkbox" class="{cls}" value="{v}" checked '
                 f'onchange="lflt()">{lab}</label>')
    H.append('</div>')
_fgroup("Market", "lmk", ["Spread", "Moneyline", "Total"])
_fgroup("Grade", "lt", ["A", "B", "C", "D"])
_fgroup("Result", "lr", ["W", "L", "P", "Open"])
_fgroup("Week", "lw", _wks)
_fgroup("Day", "ld", _dys, short=True)
_fgroup("Side", "lfv", ["Favorite", "Underdog", "Over", "Under"])
_fgroup("Venue", "lvn", ["Home", "Road"])
_fgroup("Team", "ltm", _tms)
...
H.append('</div></details>')
for _sec, _title in [("lsum", "Overall"), ("lgrade", "By grade"), ("lmkt", "By market"),
                     ("lweek", "By week"), ("lgrid", "Team by market"),
                     ("lclv", "Closing line value")]:
    H.append(f'<h3 class="sd">{_title}</h3><div id="{_sec}" class="scroll"></div>')
H.append('<h3 class="sd" id="s-picks">Every pick</h3><div id="lrows" class="scroll"></div>')
H.append('</div>')
...
<p>The dropdown at the top switches between three views: the <b>Weekly Card</b>, the <b>Team Cheat Sheet</b>, and <b>Ledger and Results</b>, which holds every pick of the season with breakdowns by grade, market, week and team, a closing line value summary, and a CSV export of whatever the filters leave on screen.</p>
...
<script>
var LROWS = {json.dumps(ROWS)};
var LBE = {PK.BREAK_EVEN};
var NAVS={{card:[['s-issues','Needs attention'],['s-summary','Summary'],['s-changes','What changed'],['s-notes','Game notes'],['s-experts','Expert records'],['s-help','How to read']],
 ...
 ledger:[['s-ledger','Results'],['s-picks','Every pick'],['s-help','How to read']]}};
...
function recCell(w,l,p){{ var n=w+l, iv=wil(w,l);
  if(!n) return '<td class="num">'+(p?'0-0-'+p:'—')+'</td><td class="num">—</td><td class="num">—</td>';
  var pct=100*w/n, cls = iv&&iv[0]>LBE ? 'rw' : (iv&&iv[1]<LBE ? 'rl' : '');
...
function lmatch(){{
  var g={{lmk:'market',lt:'grade',lr:'res',lw:'wk',ld:'day',lfv:'fav',lvn:'venue'}};
  var sel={{}}, all={{}};
  Object.keys(g).forEach(function(c){{ sel[c]=lsel(c); all[c]=document.querySelectorAll('.'+c).length; }});
  var tm=lsel('ltm'), tmAll=document.querySelectorAll('.ltm').length;
  return LROWS.filter(function(r){{
    for(var c in g){{ if(sel[c].length===0) return false;
      if(sel[c].length!==all[c] && sel[c].indexOf(String(r[g[c]]))<0){{
        if(!(g[c]==='fav' && (r.market==='Total') && sel[c].indexOf(r.side)>-1)) return false; }} }}
    if(tm.length===0) return false;
    if(tm.length!==tmAll && tm.indexOf(r.away)<0 && tm.indexOf(r.home)<0) return false;
    return true;}});}}
var LCUR=[];
function lflt(){{
  var R = lmatch(); LCUR = R;
  var t=tally(R);
  document.getElementById('lsum').innerHTML = tbl('<th>All picks</th>'+RECHEAD(),
    recRow('Season to date', R) + (t.o? '<tr><td>Still open</td><td class="num" colspan="4">'+t.o+' pick(s) not settled</td></tr>':''));
  var gh='';
  ['A','B','C','D'].forEach(function(g){{ var rs=R.filter(function(r){{return r.grade===g}});
    if(rs.length) gh += recRow('<span class="tag t-'+g.toLowerCase()+'">'+g+'</span>', rs);}});
  document.getElementById('lgrade').innerHTML = tbl('<th>Grade</th>'+RECHEAD(), gh || '<tr><td>No picks</td></tr>');
  var mh='';
  ['Spread','Moneyline','Total'].forEach(function(m){{ var rs=R.filter(function(r){{return r.market===m}});
    if(rs.length) mh += recRow(m, rs);}});
  document.getElementById('lmkt').innerHTML = tbl('<th>Market</th>'+RECHEAD(), mh || '<tr><td>No picks</td></tr>');
  var wk=[...new Set(R.map(function(r){{return r.wk}}))].sort(function(a,b){{return a-b}}), wh='';
  wk.forEach(function(w){{ wh += recRow('Week '+w, R.filter(function(r){{return r.wk===w}}));}});
  document.getElementById('lweek').innerHTML = tbl('<th>Week</th>'+RECHEAD(), wh || '<tr><td>No picks</td></tr>');
  var tms={{}};
  R.forEach(function(r){{ [r.away,r.home].forEach(function(t){{ (tms[t]=tms[t]||[]).push(r); }}); }});
  var names=Object.keys(tms).sort(function(a,b){{
    var f=function(x){{var t=tally(tms[x]);return t.w-t.l}}; return f(b)-f(a);}});
  var gh2='';
  names.forEach(function(t){{
    var cells='';
    ['Spread','Moneyline','Total'].forEach(function(m){{
      var x=tally(tms[t].filter(function(r){{return r.market===m}}));
      cells += '<td class="num">'+(x.w+x.l+x.p ? x.w+'-'+x.l+(x.p?'-'+x.p:'') : '—')+'</td>';}});
    var a=tally(tms[t]);
    gh2 += '<tr><td class="game"><span class="tm-'+t+'">'+t+'</span></td>'+cells
        + recCell(a.w,a.l,a.p)+'<td class="num">'+(a.w+a.l+a.p)+'</td></tr>';}});
  document.getElementById('lgrid').innerHTML = tbl(
    '<th>Team</th><th>Spread</th><th>Moneyline</th><th>Total</th>'+RECHEAD(), gh2 || '<tr><td>No picks</td></tr>');
  var cl=R.filter(function(r){{return r.clv!==null&&r.clv!==undefined}});
  var toward=cl.filter(function(r){{return r.clv>0}}).length, away=cl.filter(function(r){{return r.clv<0}}).length;
  var avg=cl.length? cl.reduce(function(a,r){{return a+r.clv}},0)/cl.length : 0;
  document.getElementById('lclv').innerHTML = tbl(
    '<th>Closing line value</th><th>Value</th>',
    cl.length ? '<tr><td>Moved toward our side</td><td class="num">'+toward+' of '+cl.length
      +' ('+(100*toward/cl.length).toFixed(1)+'%)</td></tr>'
      +'<tr><td>Moved away</td><td class="num">'+away+'</td></tr>'
      +'<tr><td>Average move in our favour</td><td class="num">'+avg.toFixed(2)+' pts</td></tr>'
      : '<tr><td colspan="2">No opening numbers on file yet</td></tr>');
  var rh='';
  R.slice().sort(function(a,b){{ return b.wk-a.wk || a.game.localeCompare(b.game) || a.market.localeCompare(b.market);}})
   .forEach(function(r){{
    var rc = r.res==='W'?'rw':(r.res==='L'?'rl':(r.res==='P'?'rp':''));
    rh += '<tr><td class="num">'+r.wk+'</td><td class="num">'+String(r.date).slice(5).replace('-','/')+'</td>'
       + '<td class="num">'+String(r.day).slice(0,3)+'</td><td class="game">'+r.game+'</td>'
       + '<td>'+r.market+'</td><td class="pick">'+r.pick+'</td><td class="num">'+r.our+'</td>'
       + '<td class="num">'+r.mkt+'</td><td class="num">'+r.gap+'</td>'
       + '<td class="num">'+(r.clv===null||r.clv===undefined?'—':(r.clv>0?'+':'')+r.clv)+'</td>'
       + '<td><span class="tag t-'+String(r.grade).toLowerCase()+'">'+r.grade+'</span></td>'
       + '<td class="'+rc+'">'+(r.res==='Open'?'—':r.res)+'</td></tr>';}});
  document.getElementById('lrows').innerHTML = tbl(
    '<th>Wk</th><th>Date</th><th>Day</th><th>Game</th><th>Market</th><th>Pick</th><th>Our #</th>'
    +'<th>Market #</th><th>Gap</th><th>CLV</th><th>Grade</th><th>Result</th>', rh || '<tr><td>No picks match these filters</td></tr>');
  var c=document.getElementById('lcount');
  if(c) c.textContent = R.length+' of '+LROWS.length+' picks shown, '+(t.w+t.l+t.p)+' settled';
}}
function lAll(o){{[].slice.call(document.querySelectorAll('.lmk,.lt,.lr,.lw,.ld,.lfv,.lvn,.ltm')).forEach(function(e){{e.checked=!!o}});lflt();}}
function lcsv(){{
  var cols=['wk','date','day','game','market','pick','our','mkt','gap','clv','grade','res'];
  var out=[cols.join(',')].concat(LCUR.map(function(r){{
    return cols.map(function(c){{ var v=r[c]===null||r[c]===undefined?'':String(r[c]);
      return /[",]/.test(v) ? '"'+v.replace(/"/g,'""')+'"' : v; }}).join(',');}})).join('\\n');
  var a=document.createElement('a');
  a.href='data:text/csv;charset=utf-8,'+encodeURIComponent(out);
  a.download='nfl-card-results.csv'; a.click();}}
