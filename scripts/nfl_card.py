...
# Grading scale, read once. Named at module level so the two functions that use
# them cannot pick up different values.
G_WINPTS = float(MS.GRADING["win_points"])
G_VALPTS = float(MS.GRADING["value_points"])
G_PFLOOR = 100.0 * float(MS.GRADING["p_floor"])
G_PCEIL = 100.0 * float(MS.GRADING["p_ceil"])
G_VALCEIL = float(MS.GRADING["value_ceil"])
G_A = float(MS.GRADING["grade_a"])
G_B = float(MS.GRADING["grade_b"])
G_C = float(MS.GRADING["grade_c"])
G_GATE_A = float(MS.GRADING["gate_a"])
G_GATE_B = float(MS.GRADING["gate_b"])
G_GATE_D = float(MS.GRADING["gate_d"])
...
if "Prior2026" in r.columns:
    _drift = float((r["rating"] - r.Prior2026).abs().max())
...
for _,g in gw.iterrows():
    ...
    # Every number below comes from model_state.json. They were literals here,
    # which meant the grading scale could not be changed, checked or even found
    # without reading this function. See 4.3h.
    #
    # WHAT THESE ARE: decisions, not findings. There is no backtest behind this
    # card, so nothing here is measured. The college card grades on the same
    # scheme and had to add a fourth gate, because running it there unchanged
    # graded most of the slate A on a strategy that backtested BELOW break-even.
    # Moving the endpoints only changes how many A grades get printed; it cannot
    # make them mean anything. This card has no such cap yet, and until it has a
    # record the letters are claims nothing stands behind.
    def _raw_score(edge_pp, winp):
        """Composite score, 0-100, unrounded.
        edge_pp = simulated probability minus the de-vigged market probability,
                  in probability points
        winp    = simulated probability this specific pick wins, 0-1"""
        w = 100 * winp
        W = float(np.clip((w - G_PFLOOR) / (G_PCEIL - G_PFLOOR), 0, 1)) * G_WINPTS
        E = float(np.clip(edge_pp / G_VALCEIL, 0, 1)) * G_VALPTS
        return W + E

    def combo_score(edge_pp, winp):
        """The score as DISPLAYED, rounded to one decimal."""
        return round(_raw_score(edge_pp, winp), 1)

    def combo(edge_pp, winp):
        """The letter for that score, after the value gates.

        The gates stop a heavily priced favourite grading well while offering
        nothing over the price: a high win probability alone is not an edge.

        Graded on the UNROUNDED score, which is what this has always done. It
        matters at the boundary: a score of 39.975 displays as 40.0 and grades
        D, because 39.975 is below the C band. Rewriting this to grade on the
        rounded number changed 59 of 442,401 tested combinations, which is how
        it was caught. Left as it was; whether the display and the grade should
        agree at the boundary is a decision to take on purpose, not a side
        effect of moving constants into a file.
        """
        score = _raw_score(edge_pp, winp)
        g = ("A" if score >= G_A else "B" if score >= G_B
             else "C" if score >= G_C else "D")
        if edge_pp < G_GATE_A and g == "A": g = "B"
        if edge_pp < G_GATE_B and g in ("A", "B"): g = "C"
        if edge_pp < G_GATE_D: g = "D"
        return g
...
.pk-a{{background:var(--abg);color:var(--a)}}.pk-b{{background:var(--bbg);color:var(--b)}}
.pk-c{{background:var(--cbg);color:var(--c)}}.pk-d{{background:var(--dbg);color:var(--d)}}
...
for _,x in D.iterrows():
    ...
    if x.gid in CHANGES:
        _c = CHANGES[x.gid]
        reads.append(f'<span class="chgTag">news updated</span> '
                     f'{len(_c["added"])} added, {len(_c["removed"])} removed')
...
if CHANGES:
    ...
    for _g, _c in CHANGES.items():
        ...
        _aw, _hm = (_g.split("@") + ["", ""])[:2]
        H.append(f'<div class="card chg"><h3><span class="tm-{_aw}">{_aw}</span>'
                 f'<span class="vs"> at </span><span class="tm-{_hm}">{_hm}</span>{_tagtxt}</h3>')
        if _c["added"]:
...
<script>
var NAVS={{card:[['s-issues','Needs attention'],['s-summary','Summary'],['s-changes','What changed'],['s-notes','Game notes'],['s-help','How to read']],
...
function buildNav(v){{
 ...
 [].slice.call(document.querySelectorAll('.secnav')).forEach(function(el){{
   ...
   var all=[['s-summary','Summary'],['s-changes','What changed'],['s-notes','Game notes'],['s-cheat','Conference ranks'],
